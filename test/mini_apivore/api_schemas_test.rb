...
class UnimplementedPathTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/02_unimplemented_path.json", __FILE__) )
...
class MismatchedTypeTest < ActionDispatchMocker
  ...
  test 'mismatched property' do
    ...
    match?
    assert( @errors[0] == " '/api/services.json#/0/name' of type string did not match one or more of the following types: integer, null " )
...
class UnexpectedHttpTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/04_unexpected_http_response.json", __FILE__) )
...
class ExtraPropertiesTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/05_extra_properties.json", __FILE__) )

  test 'extra property' do
    prepare_action_env(:get, "/services.json", 200)
    match?
    assert( @errors[0] == " '/api/services.json#/0' contains additional properties [\"name\"] outside of the schema when none are allowed " )

    prepare_action_env(:get, "/services/{id}.json", 200, { "id" => 1})
    match?
    assert( @errors[0] == " '/api/services/1.json#/' contains additional properties [\"name\"] outside of the schema when none are allowed " )
...
class MissingPropertiesTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/06_missing_required_property.json", __FILE__) )
...
class MissingNonRequiredPropertiesTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/07_missing_non-required_property.json", __FILE__) )
...
class CorrectResponsesTest < ActionDispatchMocker
  ...
  init_swagger( File.expand_path("../../data/01_sample2.0.json", __FILE__) )
  ...
  def check_for_final_test
    ...
    assert( MiniApivore.prepare_untested_errors[0] == 'get /services.json is untested for response code 200 in test class CorrectResponsesTest' )
